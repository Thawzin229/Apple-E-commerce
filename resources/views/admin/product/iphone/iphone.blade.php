    ...
    <div class="mdl-cell mdl-cell--12-col-desktop mdl-cell--12-col-tablet mdl-cell--4-col-phone">
        <div class="mdl-card mdl-shadow--2dp">
            <div class="mdl-card__title" id="title">
                <h1 class="mdl-card__title-text">Iphone product table
                    <span class="mx-2"><i class="zmdi zmdi-smartphone-iphone"></i></span>
                </h1>
                <form action="{{ route('admin#iphoneproductpage') }}" method="get">
                    @csrf
                ...
                <td class="mdl-data-table__cell--non-numeric"><a href="{{ route('admin#createiphonepage') }}"><button
                            class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect button--colored-teal">Add
                            iphone</button></a></td>
            </div>
                        ...
                        @if (count($data) !== 0)
                            @foreach ($data as $item)
                                <tr>
                                    ...
                                    <td class="mdl-data-table__cell--non-numeric text-center">{{ $item['category_name'] }}
                                    </td>
                                    ...
                                    <td class="mdl-data-table__cell--non-numeric text-center">{{ $item['specification'] }}
                                    </td>
                                    <td class="mdl-data-table__cell--non-numeric text-center">{{ $item['description'] }}
                                    </td>
                                    <td class="mdl-data-table__cell--non-numeric text-center"><img
                                            src="{{ asset('storage/' . $item['image']) }}" class="productimage"
                                            alt=""></td>
                                    ...
                                    <td class="mdl-data-table__cell--non-numeric text-center">
                                        <a href="{{ route('admin#deleteproduct', $item['id']) }}"
                                            class="text-decoration-none"><span
                                                class="label label--mini background-color--secondary p-3"><i
                                                    class="zmdi zmdi-delete me-2"></i>Delete</span></a>
                                        <a href="{{ route('admin#updatepage', $item['id']) }}"
                                            class="text-decoration-none text-light "><span
                                                class="label label--mini background-color--primary p-3"><i
                                                    class="zmdi zmdi-border-color ms-2"></i>Edit</span></a>
                                    </td>
                                </tr>
                            @endforeach
                        @else
                            <h4 class="text-center p-5">No result found <span
                                    class="text-danger">{{ request('searchval') }}</span></h4>
                        @endif
                    </tbody>
                </table>
